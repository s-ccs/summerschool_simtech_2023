...
# ╔═╡ 23254086-5d3f-11ee-02be-a32f9812c6a0
begin
	using CairoMakie
end

# ╔═╡ 0ab3a492-b3cb-4605-8d25-553ed9b174df
using StableRNGs

# ╔═╡ 8b95f8d6-1a99-4493-9ce2-94344e321dcb
using PlutoUI

# ╔═╡ 911fb08e-99f6-4f87-bbf8-3d460966de44
using PlutoExtras

# ╔═╡ 68b8eed7-94a7-46d5-be1c-5be614f097f5
PlutoExtras.BondTable([PlutoExtras.@BondsList "Sliders" let 
"nmax" = @bind(nmax,PlutoUI.Slider(1:500))
"β" = @bind(β, PlutoUI.Slider(-5:5))
end]
)

# ╔═╡ b85e9e0e-a554-465f-8888-097d36075841


# ╔═╡ 4c273ad2-7b68-4384-b909-3419748b5bd3
function simulate(seed,nmax)
	return 10 .* randn(StableRNG(seed),nmax) .+ β
end
...
#set_theme!()
	...
	ax = f[1,1] = Axis(f)
scatter!(ax,cumsum(y) ./ (1:length(y)),color=1:length(y),colormap=:Reds)
	hlines!([β],color=:black)
	xlims!(ax,-10,550)

	ax_d,d = hist(f[1,2],y_mean,direction=:x,bins=-10:0.1:10)
	linkyaxes!(ax_d,ax)
	ylims!(ax_d,-10,10)
	hidespines!.([ax,ax_d],:r,:t)

	ax.xgridvisible = false
	ax_d.xgridvisible = false
	ax_d.xticksvisible = false
	ax_d.xticklabelsvisible = false
	ax_d.bottomspinevisible = false
	colsize!(f.layout,2,Relative(0.2))

	Label(f[1,1,TopLeft()],"A",padding=[0,0,5,0])
	Label(f[1,1,TopLeft()],"⭕",padding=[0,1.5,8,0],fontsize=30)
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "f2fe61bd66b16b42252fca447d001ee7fe7e5874"
